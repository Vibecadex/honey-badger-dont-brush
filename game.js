...
(() => {
  ...
  const SPRITE_SLOTS = {
    ...
    // `body` is the always-visible base. `headPos` and `headSize` describe WHERE
    // in the body image the head sits (normalized 0..1, relative to drawn size)
    // so state-specific head overlays can snap onto it and hide the base head.
    // Tune these after seeing the render:
    //   headPos.x → 0 = far left of body image, 1 = far right
    //   headSize  → head overlay width as fraction of body width
    body: {
      src: 'assets/badger_body.jpg',
      anchor: { x: 0.5, y: 0.5 },
      clip: 'ellipse',
      headPos: { x: 0.70, y: 0.52 },
      headSize: 0.26,
    },
    // Head overlays — drawn on top of the body at its headPos. Soft clip blends edges.
    // Watching/biting are CC-licensed Wikimedia SVG illustrations of aggressive poses
    // (the "Cape ratel" JPEG turned out to be a 19th-century engraving, not a photo).
    head_away:     { src: 'assets/badger_head_away.jpg',     anchor: { x: 0.5, y: 0.5 }, clip: 'soft' },
    head_turning:  { src: 'assets/badger_head_turning.jpg',  anchor: { x: 0.5, y: 0.5 }, clip: 'soft' },
    head_watching: { src: 'assets/badger_head_watching.svg', anchor: { x: 0.5, y: 0.5 }, clip: 'soft' },
    head_biting:   { src: 'assets/badger_head_biting.svg',   anchor: { x: 0.5, y: 0.5 }, clip: 'soft' },
    tail:          { src: 'assets/badger_tail.png',          anchor: { x: 0.0, y: 0.5 } },
    comb:          { src: 'assets/comb.png',                 anchor: { x: 0.5, y: 0.3 } },
    background:    { src: 'assets/background.jpg',           anchor: { x: 0.5, y: 0.5 } },
  };
  ...
  function loadSprites() {
    for (const [slot, def] of Object.entries(SPRITE_SLOTS)) {
      ...
      img.onerror = () => { /* missing file: silently fall back to shape rendering */ };
      img.src = def.src;
    }
  }
  ...
  // Pick head-overlay slot + alpha for the current state.
  // SAFE = no overlay (base photo's head visible). Other states fade in a
  // head photo positioned over the body's head, hiding the base head.
  function pickHeadOverlay(state) {
    const hasTurning = !!sprites.head_turning;
    const watchingSlot = sprites.head_watching ? 'head_watching' : null;
    const bitingSlot = sprites.head_biting ? 'head_biting' : watchingSlot;

    if (state === STATE.TURNING) {
      const t = Math.min(1, game.stateTimer / Math.max(1, nextTimings.turning));
      return { slot: hasTurning ? 'head_turning' : watchingSlot, alpha: t };
    }
    if (state === STATE.WATCHING) return { slot: watchingSlot, alpha: 1 };
    if (state === STATE.BITING)  return { slot: bitingSlot,    alpha: 1 };
    if (state === STATE.SAFE && game.returnAnim.active) {
      const rt = Math.min(1, game.returnAnim.t / game.returnAnim.dur);
      return { slot: watchingSlot, alpha: 1 - rt };
    }
    return { slot: null, alpha: 0 };
  }
  ...
  function drawBadger() {
    ...
    if (sprites.body) {
      // Dramatic zoom during BITING.
      const biteT = state === STATE.BITING ? Math.min(1, game.stateTimer / 400) : 0;
      const zoom = 1 + biteT * 0.18;
      const bodyW = b.bodyRx * 2.2 * zoom;
      drawSprite('body', b.cx, b.cy, bodyW);

      // Head overlay on top of body's own head.
      const bodyDef = SPRITE_SLOTS.body;
      const headPos = bodyDef.headPos || { x: 0.3, y: 0.45 };
      const headSizeFrac = bodyDef.headSize || 0.35;
      const bodyImg = sprites.body;
      const bodyH = bodyW * (bodyImg.naturalHeight / bodyImg.naturalWidth);
      const headX = b.cx + (headPos.x - 0.5) * bodyW;
      const headY = b.cy + (headPos.y - 0.5) * bodyH;
      const headW = bodyW * headSizeFrac;

      const overlay = pickHeadOverlay(state);
      if (overlay.slot && sprites[overlay.slot] && overlay.alpha > 0.01) {
        ctx.save();
        ctx.globalAlpha = overlay.alpha;
        drawSprite(overlay.slot, headX, headY, headW);
        ctx.restore();
      }
  ...
  // ----- Start wiring -----
  // NOTE: TB2 is the pre-PNG-art fork. Sprites intentionally NOT loaded — everything
  // renders via placeholder shapes. To re-enable real art later, uncomment:
  //   loadSprites();
